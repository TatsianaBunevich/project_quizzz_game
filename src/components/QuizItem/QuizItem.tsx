...
const QuizItem = ({ children, className }: React.ComponentProps<'div'>) => {
	return (
		<div className={`${styles.quizItem} ${className ?? ''}`}>
			{children}
		</div>
	);
};
...
const Title = ({ title, className }: React.ComponentProps<'h2'> & { title: string }) => <h2 className={`${styles.title} ${className ?? ''}`} dangerouslySetInnerHTML={{__html: title}} />;
