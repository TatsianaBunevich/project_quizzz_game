...
> = (set, get) => ({
    ...
    set(
      (state) => {
        ...
        quizItemsForSort
          ...
          .forEach((item) => item.answers.sort(() => Math.random() - 0.5))
  ...
  resetQuiz: () => {
    set({ ...initialQuizState }, undefined, 'quiz/resetQuiz')
  },
  ...
  handleNewTry: () => {
    set(
      () => ({ activeId: initialQuizState.activeId }),
      undefined,
      'quiz/resetActiveId'
    )
    set(
      (state) => {
        state.quizItems.find((q) =>
          q.answers.find((ans) => (ans.isSelected = false))
        )
      },
      undefined,
      'quiz/resetQuizItems'
